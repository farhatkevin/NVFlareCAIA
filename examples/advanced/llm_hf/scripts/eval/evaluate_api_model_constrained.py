...
def load_dataset_files(dataset_paths: List[str], max_tokens: int = 4000):
    ...
    dataset = (
        datasets.load_dataset("json", data_files=dataset_paths, split="train").shuffle(seed=42).select(range(2500))
    )
    ...
    print(
        f"Filtered dataset: {original_size} -> {filtered_size} examples ({original_size - filtered_size} removed due to length)"
    )
...
async def make_api_request(
    ...
    for attempt in range(max_retries):
        try:
            async with session.post(config.base_url, headers=config.headers, json=request_data) as response:
                if response.status == 200:
                    ...
                    if debug:
                        ...
                        if "usage" in response_data:
                            ...
                            print(
                                f"Token usage: prompt={usage.get('prompt_tokens', 'N/A')}, completion={usage.get('completion_tokens', 'N/A')}, total={usage.get('total_tokens', 'N/A')}"
                            )
...
async def evaluate_model_on_dataset(
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        for batch_start in tqdm(range(0, dataset_size, batch_size), desc="Evaluating"):
            ...
            batch_ids = (
                list(batch["id"]) if "id" in batch else [f"item_{batch_start + i}" for i in range(len(batch_prompts))]
            )
